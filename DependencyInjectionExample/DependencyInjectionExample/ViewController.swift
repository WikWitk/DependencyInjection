//
//  ViewController.swift
//  DependencyInjectionExample
//
//  Created by Wiktor Witkowski on 04/04/2024.
//

import UIKit
import Swinject

class ViewController: UIViewController {
    
    
    //Swinject
    let container : Container = {
       let container = Container()
        container.register(BackgroundProviderClass.self) { resolver in
            return BackgroundProviderClass()
        }
        
        container.register(AnotherViewController.self) { resolver in
            let vc = AnotherViewController(providerProtocol: resolver.resolve(BackgroundProviderClass.self))
            return vc
        }
        
        
        return container
    }()

    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view.
        view.backgroundColor = .systemCyan
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 250, height: 50))
        button.setTitle("Open Another VC", for: .normal)
        button.center = view.center
        button.addTarget(self, action: #selector(buttonClicked), for: .touchUpInside)
        view.addSubview(button)
    }
    
    @objc private func buttonClicked(){
        
        if let viewController = container.resolve(AnotherViewController.self){
            present(viewController, animated: true)
        }
        
    }


}

